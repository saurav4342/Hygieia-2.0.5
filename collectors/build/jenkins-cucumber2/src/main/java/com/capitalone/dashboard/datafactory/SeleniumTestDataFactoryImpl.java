...
import com.capitalone.dashboard.collector.SeleniumSettings;
...
@Component
public class SeleniumTestDataFactoryImpl implements SeleniumTestDataFactory{
	private SeleniumSettings jenkinsSettings;
	
	@Autowired
	public SeleniumTestDataFactoryImpl(SeleniumSettings jenkinsSettings){
	...
	public List<TestResult> getTestResult() throws IOException{
        ...
        for (TestCapability cap : capabilities) {
            ...
            testResult.setTotalCount(cap.getTotalTestCaseCount());
        testResult.setSuccessCount(cap.getSuccessTestCaseCount());
        testResult.setFailureCount(cap.getFailedTestCaseCount());
	...
	public List<TestCapability> getTestCapabilities() throws IOException{
	    ...
	    while (it.hasNext()) {
	        ...
	        long duration = 0l;
	        int testSuiteSkippedCount = 0, testSuiteSuccessCount = 0, testSuiteFailCount = 0, testSuiteUnknownCount = 0,totalTestCaseCount=0,failedTestCaseCount=0,successTestCaseCount=0;
	        ...
	        for (TestSuite t : suites) {
	        	...
	        	if(endTime < t.getEndTime()){
	             ...
	             totalTestCaseCount+=t.getTotalTestCaseCount();
	             failedTestCaseCount+=t.getFailedTestCaseCount();
	             successTestCaseCount+=t.getSuccessTestCaseCount();
	             if(t.getStatus()!=null){
	             switch (t.getStatus()) {
	                 case Success:
	         ...
	         testCap.setFailedTestSuiteCount(failedTestCaseCount);
	         testCap.setSkippedTestSuiteCount(testSuiteSkippedCount);
	         testCap.setSuccessTestSuiteCount(successTestCaseCount);
	         testCap.setUnknownStatusTestSuiteCount(testSuiteUnknownCount);
	         testCap.setTotalTestSuiteCount(totalTestCaseCount);
	         ...
	         testCap.setTotalTestCaseCount(totalTestCaseCount);
	         testCap.setFailedTestCaseCount(failedTestCaseCount);
	         testCap.setSuccessTestCaseCount(successTestCaseCount);
	...
	public void getTestSuitesFromFile() throws IOException{
		...
		InputStream files = urlc.getInputStream();
		BufferedReader reader = new BufferedReader(new InputStreamReader(files));
		 getTestSuiteWithCases(reader,timestamp,version);
		}
	}
	
	@SuppressWarnings("unused")
	public void getTestSuiteWithCases(BufferedReader reader,long timestamp,String version) throws IOException{
		String line = null;
        long duration=0l;
		...
		if(podMap.containsKey(version)){
			List<TestSuite> suites =podMap.get(version);
			suites.add(suite);
			podMap.put(version, suites);
		}
		else{
			List<TestSuite> suites = new ArrayList<TestSuite>();
			suites.add(suite);
