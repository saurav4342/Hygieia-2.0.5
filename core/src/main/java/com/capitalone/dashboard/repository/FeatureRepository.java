...
public interface FeatureRepository extends CrudRepository<Feature, ObjectId>,
	...
	@Query( value = "{'sNumber' : ?0 }")
	List<Feature> findbySNumber(String sNumber);
	
	@Query( value = "{ 'userStories.storyNumber' : ?0}")
	List<Feature> findUserStoryByUserStoryNumber(String userStoryNumber);
	
	@Query( value = "{ 'foundInBuild' : ?0 }")
    List<Feature> findByFoundInBuild(String teamId);
